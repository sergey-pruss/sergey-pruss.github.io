...
def update_sitemap():
    """Обновляет lastmod у главной и блога на сегодня"""
    import os
    ...
    # Update lastmod for root and blog only (posts keep their publish date)
    import re
    def update_root_date(m):
        return m.group(0).replace(re.search(r'<lastmod>[^<]+</lastmod>', m.group(0)).group(0),
                                   f'<lastmod>{today}</lastmod>')

    xml = re.sub(
        r'<url>\s*<loc>https://sergeypruss\.ru/(?:blog\.html)?</loc>.*?</url>',
        update_root_date, xml, flags=re.DOTALL
    )
